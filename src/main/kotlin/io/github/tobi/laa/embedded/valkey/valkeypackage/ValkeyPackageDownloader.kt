 ...
 *
 * @param valkeyVersion The version of the Valkey installation bundled within the package, e.g. `"9.0.3"`.
 ...
 * @param downloadUri The URI from which to download the Valkey package.
 * @param proxy The proxy to use for downloading the package (defaults to not using a proxy).
 * @param cacheDownload Whether to cache the downloaded package for future use (default is `true`).
 * @param cacheFileLocation The path where the cached package will be stored (default is a file in the system temp directory).
 * @param sha256FileChecksum The expected SHA-256 checksum of the downloaded package for integrity verification (optional).
 * @param verifyFileChecksum Whether to verify the SHA-256 checksum of the downloaded package (is enabled by default if [sha256FileChecksum] is provided).
 * @param downloadLocation The path where the downloaded package will be stored (defaults to `cacheFileLocation`).
 * @param distributionType The type of the Valkey distribution (default is [DistributionType.VALKEY]).
 */
class ValkeyPackageDownloader
@JvmOverloads
constructor(
    ...
    internal val logger = LoggerFactory.getLogger(ValkeyPackageDownloader::class.java)
    ...
    internal fun downloadPackage() {
        ...
        val connectionToMirror = if (proxy != null) {
            downloadUri.toURL().openConnection(proxy)
        } else {
            downloadUri.toURL().openConnection()
        }
        val packageSizeBytes = connectionToMirror.contentLengthLong
        var totalBytesRead: Long = 0
        connectionToMirror.getInputStream().use { downloadStream ->
            targetPath.parent.createDirectories()
            newOutputStream(targetPath, TRUNCATE_EXISTING, CREATE).use { targetStream ->
                totalBytesRead =
                    downloadValkeyDistributionAndLogProgress(downloadStream, targetStream, packageSizeBytes)
            }
            logger.info(
    ...
    private fun downloadValkeyDistributionAndLogProgress(
        ...
        while (downloadStream.read(buffer).also { bytesRead = it } != -1) {
            ...
            val progress = (totalBytesRead * 100) / packageSizeBytes
