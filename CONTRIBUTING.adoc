= Contributing to `embedded-valkey`

`embedded-valkey` is released under the Apache 2.0 license.
...
The artifact version should correspond to the aforementioned tag, meaning a tag such as `v1.0.0` should only be applied if the artifact version within the project's `pom.xml` is `1.0.0`.
...
[,bash]
----
./mvnw release:prepare release:clean -B
----
